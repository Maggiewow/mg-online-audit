...
import { uploadInit, uploadFinish } from '@/api/webupload'
export default {
  name: 'WebUpload',
  props: {
    accept: {
      ...
      new WebUploader.Uploader.register(
        ...
        },
        {
          ...
          beforeSend: function (block) {
            ...
            if (chunk <= file.current_chunk) {
              // 若当前分片小于已传分片,表明，返回失败给WebUploader，表明该分块不需要上传
              task.reject()
            }
            task.resolve()
            return task.promise()
      ...
      that.uploader.on('error', (type) => {
        ...
        if (type === 'F_EXCEED_SIZE') {
          errorMessage = `文件大小不能超过${
            that.fileSingleSizeLimit / (1024 * 1000)
          }M`
