 ...
 * @LastEditTime: 2023-01-13 11:25:39
 * @LastEditors: 赵婷婷
...
<script>
import {
  getStatusArticleList,
  getAllRelatedSeriesList,
  getSeriesCates,
} from '@/api/manu'
...
export default {
  ...
  methods: {
    ...
    chooseDraftOk() {
      ...
      if (this.tabKey === '1') {
        ...
        choosedIndexs.forEach((index) => {
          const { series_id, source_id, remote_item_id } =
            this.indexIdObj[index] || {}
          source_id &&
            choosedIds.push({
              series_id,
    ...
    chooseData(choosedDraft) {
      ...
      let otherPageIds = this.artSelected.filter(
