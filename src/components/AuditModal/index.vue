 ...
 * @LastEditors: 赵婷婷
 * @LastEditTime: 2023-06-24 10:06:40
-->
<template>
	<div>
		<Modal
...
export default {
	...
	methods: {
		...
		setRightClick() {
			...
			this.$nextTick(() => {
				if (this.allowCopy) {
					$('#myVideo').unbind('contextmenu', this.hideRight)
				} else {
					$('#myVideo').bind('contextmenu', this.hideRight)
				}
			})
		},
			...
			getSucaiVersionDetail(file_id, initVersion)
				.then((res) => {
					...
					if (res.status === 200 && file_id === this.fileId) {
						...
						// 横屏视频 url = 'https://stream7-transcode.iqilu.com/1/sucaiku/202106/27/94267d670bd446d68cf1df95abf6fa55.mp4';
		...
		handleDownload(type = 'lowcode') {
			let methodObj = {
				lowcode: getVersionTranscodeUrl,
				origin: getVersionOriginUrl,
			}
